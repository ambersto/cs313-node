...
function getEventsFromDB(callback) {
	...
	var sql = "SELECT e.id AS id, e.event_name AS event_name, e.event_date AS event_date, v.venue_name AS venue_name, a.first_name AS first_name, e.notes AS notes FROM event e INNER JOIN venue v ON e.venue_id=v.id INNER JOIN author a ON e.author_id=a.id";
